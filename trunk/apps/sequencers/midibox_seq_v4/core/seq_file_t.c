// $Id: seq_file_t.c 1883 2013-12-10 21:33:05Z tk $
/*
...
#define FLUSH_BUFFER if( !write_to_file ) { DEBUG_MSG(line_buffer); } else { status |= FILE_WriteBuffer((u8 *)line_buffer, strlen(line_buffer)); }
  ...
  sprintf(line_buffer, "FxMIDI_NumChannels %d\n", tcc->fx_midi_num_chn);
  FLUSH_BUFFER;

  sprintf(line_buffer, "FxMIDI_Mode %d\n", tcc->fx_midi_mode.beh);
  FLUSH_BUFFER;

  sprintf(line_buffer, "FxMIDI_FwdNonNotes %d\n", tcc->fx_midi_mode.fwd_non_notes);
  FLUSH_BUFFER;
